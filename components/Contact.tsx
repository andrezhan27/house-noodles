...
const deliveries = [
  { name: "Glovo", href: "https://glovoapp.com/pt/pt/lisboa/stores/azul-sublime-lda-lis", accent: "#e0a800", surface: "#fff3cc", ink: "#5f4700" },
  { name: "Uber Eats", href: "https://www.ubereats.com/pt/store/house-noodles-entrecampos/nvYmKvnuRpOZS8u2PSWs5A", accent: "#06c167", surface: "#e7f8ef", ink: "#075c32" },
  { name: "Bolt Food", href: "https://food.bolt.eu/en/386-lisbon/p/892572-house-noodles-entrecampos/", accent: "#2f8b57", surface: "#e8f5ee", ink: "#195237" },
];
  ...
  return (
    <section id="contact" className="bg-blue py-24 text-white md:py-32">
      <div className="@container mx-auto max-w-[1440px] px-5 md:px-10 lg:px-16">
        ...
        <div className="mt-12 grid min-h-[680px] overflow-hidden rounded-[2rem] bg-[#f8f2e8] text-ink shadow-[0_30px_100px_rgba(0,0,0,.2)] @min-[900px]:min-h-[520px] @min-[900px]:grid-cols-[.92fr_1.08fr] md:mt-16 md:rounded-[2.5rem]">
          <div className="flex p-6 sm:p-9 @min-[900px]:p-10 lg:p-12">
                ...
                <div className="flex flex-wrap gap-2">
                  {deliveries.map((delivery) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-bold tracking-wider uppercase transition hover:-translate-y-0.5 hover:brightness-95"
                      style={{ backgroundColor: delivery.surface, borderColor: delivery.accent, color: delivery.ink }}
                    >
                      <span aria-hidden="true" className="size-2 rounded-full" style={{ backgroundColor: delivery.accent }} />
                      {delivery.name}
                    </a>
                  ))}
                </div>
...
function ContactItem({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) {
  return (
    <div className="grid gap-3 py-6 first:pt-0 last:pb-0 sm:grid-cols-[132px_1fr] sm:gap-6 @min-[900px]:grid-cols-[138px_1fr] @min-[900px]:gap-5 @min-[900px]:py-5 lg:grid-cols-[150px_1fr] lg:gap-6">
