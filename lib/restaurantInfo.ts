...
export const reservationWidgetUrl = "https://reserve.intelis.pt/housenoodles";
export const restaurantInfo = { databaseId: "housenoodles" } as const;
...
export async function getRestaurantInfo(): Promise<RestaurantInfo> {
  ...
  endpoint.searchParams.set("select", "terms_and_conditions_url,privacy_policy_url");
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`Unable to load restaurant legal links from Supabase (${response.status}).`);
      return fallback;
    }
    ...
    return {
      ...
      termsAndConditionsUrl: getSafeDocumentUrl(restaurant?.terms_and_conditions_url),
    };
  } catch (error) {
